...
const OptionButton: React.FC<OptionButtonProps> = ({ label, value, selected, onSelect }) => (
  <button
    ...
    className={`w-full py-4 text-left rounded-xl transition-transform duration-200 mb-4 flex items-center justify-between px-6 border ${
      selected
        ? 'bg-white text-black border-transparent shadow-lg'
        : 'bg-transparent text-gray-300 border-gray-600 hover:bg-gray-800 hover:scale-105'
    }`}
  >
...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, current, total, answer, onAnswer }) => {
  ...
  return (
    <div className="bg-gray-800 p-10 rounded-2xl shadow-2xl w-full max-w-3xl">
      ...
      <p className="text-2xl mb-8 font-semibold text-white leading-relaxed">{question}</p>
...
const Summary: React.FC<SummaryProps> = ({ answers }) => {
  ...
  return (
    <div className="bg-white p-12 rounded-2xl shadow-2xl w-full max-w-3xl text-center">
      <h2 className="text-3xl font-bold mb-4 text-gray-900">¡Cuestionario Completado!</h2>
      <p className="text-lg text-gray-700 mb-6">Tu puntuación total es <span className="font-semibold">{score}</span> de {answers.length * 4} posibles.</p>
      <p className="text-gray-600">Gracias por tu tiempo. Un profesional se pondrá en contacto contigo para evaluar los resultados.</p>
    </div>
  );
};
        ...
        {!isComplete ? (
          <>
            ...
            <button
              ...
              className="mt-8 px-10 py-4 rounded-full bg-white text-black font-semibold disabled:opacity-50 hover:scale-105 transition-transform duration-200"
            >
